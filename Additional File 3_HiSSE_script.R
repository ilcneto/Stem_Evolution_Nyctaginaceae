...
# check if tree tip names and data names coincide.
...
sum(habit == 0) # 82. So proportion is 82/569=0.14
sum(habit == 1) # 22; 22/44=0.5
f<-c(0.14,0.5)
...
# Model 3. HiSSE: turnove different for all the states (observed and hidden).
turnover <-c(1,2,3,4)
...
# than a parameters than the Model 3-HiSSE so this evaluates the character-independent 
# model as a null model to which compare the HiSSE model.
...
models_habit
#        loglik     AICc
#CID2  -354.9931 722.8522
#HiSSE -350.8528 724.0713
#CID4  -354.6957 726.9073
#Null  -366.7162 741.8364
#BiSSE -366.6473 743.9068
...
# 0 and 1 states.
sum(eustele == 0) # 13, so 13/183=0.07
...
# Given that three species have unknown state, we will prune the tree to remove these species
...
#sampling fraction
sum(sec_grow2 == 0) # 3, so the proportion is 3/44=0.068(~0.07)
...
models_sec_grow
#       loglik     AICc
#CID2  -311.2897 635.4730
#CID4  -310.0357 637.6366
#HiSSE -309.2654 640.9751
#BiSSE -317.8762 646.3840
#Null  -319.6654 647.7475
